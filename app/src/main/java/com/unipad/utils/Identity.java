...
//    地区及编码
    ...
    private static String getCheckFlag(String idCard){
        ...
        if(idCard.substring(idCard.length() - 1,idCard.length()).toLowerCase().equals("x".toLowerCase())){
    	   return idCard.substring(idCard.length()-1,idCard.length()).toLowerCase();
        }
        //取模运算，得到模值
    ...
    //判断时间合法性
    private static boolean checkDate(final String dateSource) throws ParseException{
    	if(Integer.parseInt(dateSource.substring(0,4)) > 1915 && Integer.parseInt(dateSource.substring(0,4)) < 2215){
    		  String dateStr = dateSource.substring(0,4)+"-"+dateSource.substring(4,6)+"-"+dateSource.substring(6,8);
    	        System.out.println(dateStr);
    	        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
    	        Date ndate = format.parse(dateStr);
    ...
    public static boolean checkIDCard(final String idCard){
        ...
        if(isNew){
            String checkFlag = getCheckFlag(idCard).toLowerCase();
            String theFlag = idCard.substring(idCard.length()-1,idCard.length()).toLowerCase();
